const GanttChart = {
    ...
    mounted: function () {
        if (this.seriesSet === undefined) {
            throw Error(
                'GanttChart : vue component error : series-set props not found !!',
            );
        }
    ...
    methods: {
        makeChart: function (chartDiv) {
            ...
            if (seriesVue === undefined) {
                console.error(
                    'GanttChart : vue component error : series component not found in child component list !!',
                );
                return;
            }
            ...
            for (let series of this.seriesSet) {
                if (series.color !== undefined) {
                    this.seriesColorMap[series.name] = am4core.color(
                        series.color,
                    );
                } else {
                    this.seriesColorMap[
                        series.name
                    ] = this.chart.colors.getIndex(idx);
                }
                idx++;
            }
            ...
            const data = this.data.map((d) => ({
                ...d,
                color: am4core.color(this.seriesColorMap[d.name]),
            }));
